...
class HospitalMiddleware implements MiddlewareInterface
{
    ...
    public function process(ServerRequestInterface $request, RequestHandlerInterface $handler): ResponseInterface
    {
        if ($hospitalId = $this->session->get('hospital')) {
            ...
            if ($archive) {
                return $this->responseFactory->createResponse(302)->withHeader('Location', '/hospital/archive');
            } else if($status === '0') {
                 return $this->responseFactory->createResponse(302)->withHeader('Location', '/hospital/pending');
            } 
        }
