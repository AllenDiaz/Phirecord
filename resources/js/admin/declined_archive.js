...
window.addEventListener("DOMContentLoaded", function () {
  ...
  const table = new DataTable("#declinedArchiveHospitalTable", {
    serverSide: true,
    ajax: "/admin/archive/declined/load",
    orderMulti: false,
    columns: [
      {
        sortable: false,
  ...
  document.querySelector("#declinedArchiveHospitalTable").addEventListener("click", function (event) {
      ...
      if (viewBtn) {
        ...
        const hospitalId  = deleteBtn.getAttribute("data-id")
              ...
              if (result.isConfirmed) {
                  ...
                  del(`/admin/declined/archive/${hospitalId}`)
                  .then(response => {
                    if (response.ok) {
                        table.draw()
                    }
          ...
          const hospitalId  = restoreBtn.getAttribute("data-id")
              ...
              if (result.isConfirmed) {
                  ...
                  get(`/admin/recover/declined/${hospitalId}`)
                  .then(response => {
                    if (response.ok) {
                        table.draw()
                    }
