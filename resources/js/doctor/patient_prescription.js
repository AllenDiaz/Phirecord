...
window.addEventListener("DOMContentLoaded", function () {
  ...
  const table = new DataTable("#patientPrescriptionTable", {
    serverSide: true,
    ajax: "/doctor/checkupform/load",
    orderMulti: false,
    columns: [
      { data: "patient" },
      { data: "doctor" },
      ...
      { data: "checkupDate" },
      {
        ...
        data: (row) => `
                    <div class="d-flex flex-">
                        ...
                        <a href="#">
                         <button class="ms-2 btn btn-outline-primary view-prescription-btn" data-id="${row.id}" data-photo="${ row.prescriptionImage }" ">
  ...
  document
    .querySelector("#patientPrescriptionTable")
    .addEventListener("click", function (event) {
      ...
      if (detailBtn) {
        ...
        const prescriptionImage = viewBtn.getAttribute("data-photo")

        viewPrescriptionModal._element.querySelector('#prescriptionImage').src = "/img/prescription/" +  prescriptionImage
...
function openViewPresciptionModal(modal, image) {
