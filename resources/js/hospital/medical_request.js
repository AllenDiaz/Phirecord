...
window.addEventListener("DOMContentLoaded", function () {
  ...
  const table = new DataTable("#patientMedicalRequestTable", {
    serverSide: true,
    ajax: "/hospital/medicalform/request/load",
    orderMulti: false,
    columns: [
  ...
  document.querySelector("#patientMedicalRequestTable").addEventListener("click", function (event) {
   const requestBtn = event.target.closest(".view-request-btn");
        ...
        if(requestBtn) {
        const medicalId = requestBtn.getAttribute("data-id");
         window.open(`/hospital/medicalform/pdf/${medicalId}`, `_blank`);
       
        } else if(doneBtn) {
          const medicalId = doneBtn.getAttribute("data-id");
          ...
          if (result.isConfirmed) {
            ...
            get(`/hospital/medicalform/${medicalId}/request/${requestId}`).then((response) => {
              if (response.ok) {
                table.draw();
              }
            });
