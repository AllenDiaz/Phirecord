...
window.addEventListener("DOMContentLoaded", function () {
      ...
      { data: "requestDate" },
      {
        ...
        data: (row) => `
                    <div class="d-flex flex-">
                        ...
                        <button class="ms-2 btn btn-outline-primary download-referral-btn" data-id="${row.id}">Reference Number</button>
                    </div>
                `,
      },
    ],
  ...
  document
    .querySelector("#patientRequestAdmissionTable")
  ...
  function openViewAdmissionModal(
    ...
    const patientBloodPressure = modal._element.querySelector(
      "#patientBloodPressure"
    );
    const patientTemperature = modal._element.querySelector(
      "#patientTemperature"
    );
    const patientWeight = modal._element.querySelector("#patientWeight");
    const patientRespiratoryRate = modal._element.querySelector(
      "#patientRespiratoryRate"
    );
    const patientPulseRate = modal._element.querySelector("#patientPulseRate");
    const patientOxygenSaturation = modal._element.querySelector(
      "#patientOxygenSaturation"
    );
